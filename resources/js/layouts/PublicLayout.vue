...
const page = usePage();
const site = page.props.siteConfig || { 
    contact: { email: 'ukmptq@unimal.ac.id', address: 'Sekretariat UKM PTQ' },
    hero_desc: ''
};
                ...
                <div class="flex h-16 items-center justify-between">
                        ...
                        <template v-for="link in navLinks" :key="link.route">
                            <Link :href="route(link.route)">
                                <Button variant="ghost" 
                                    ...
                                    route().current(link.active) ? 'bg-emerald-50 text-emerald-700 font-semibold' : 'text-slate-600 hover:bg-emerald-50 hover:text-emerald-700']">
                                    {{ link.label }}
                                </Button>
                            </Link>
                        </template>
                    ...
                    <div class="md:hidden">
                        <Sheet v-model:open="isMobileOpen">
                            ...
                            <SheetContent side="right" class="w-[300px] bg-white/95 backdrop-blur-xl border-l border-slate-200 p-0 flex flex-col h-full">
                                <SheetHeader class="text-left border-b border-slate-100 p-6 shrink-0">
                                ...
                                <div class="border-t border-slate-100 p-6 bg-slate-50/50 shrink-0">
                                    <a v-if="site.register_url" :href="site.register_url" target="_blank">
